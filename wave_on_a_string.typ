...
+ 在弦的波节附近（$y(x,t) approx 0$），分母趋于零导致 $e_N^text("strict") -> infinity$，这反映的是小分母放大效应而非截断精度的真实度量。

因此，本文采用初始最大振幅 $A_0$ 替代 $|y(x,t)|$ 作为归一化参考，定义*归一化绝对误差上界*：

$
  epsilon_N & = max_(x in [0, L_0], t >= 0) (|E_N (x,t)|) / A_0 \ & <= C/(N A_0) \ & = (2 L_0^2)/(x_0(L_0 - x_0) pi^2 N)
$
...
综合考虑这些因素，实际归一化绝对误差约为上述上界的 $1/2$ 左右，即约 $2%$，与基础小参数 $epsilon = A_0/L_0 approx 1.67%$ 同阶???
